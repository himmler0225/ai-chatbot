'use client'
import { Alert, Button, Card, Drawer, Flex, Image, Rate, Skeleton, Space, Tag, Typography } from 'antd'
...
export function ProductDrawer({ product, open, onClose }: Props) {
  ...
  const spid = product?.seller_product_id

  const { data: detail } = useTikiDetail(product?.id, spid, open)
  const { reviews, isPending: reviewLoading, isError: reviewError } = useTikiReviews(
    product?.id,
    spid,
    open,
  )
          ...
          <div>
            ...
            {reviewError ? (
              <Alert
                ...
                showIcon
                className="mt-2"
                message={t('utilities.product.reviewsError')}
              />
            ) : (
