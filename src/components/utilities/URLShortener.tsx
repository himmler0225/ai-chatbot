...
import {
  Button, Card, Divider, Flex, Input, message,
  Radio, Space, Tag, Tooltip, Typography, theme,
} from 'antd'
import { CopyOutlined, LinkOutlined } from '@ant-design/icons'
import { useTranslation } from 'react-i18next'
import { useShortenMutation } from '@/src/hooks/useUtilities'
import type { ShortenResult } from '@/src/types/chat'
...
export default function URLShortener() {
  ...
  return (
    <Flex vertical gap={12}>
      {ctx}
      <Input
        size="large" placeholder={t('utilities.shortener.placeholder')}
      ...
      {result && (
        <Card size="small" style={{ borderRadius: 10, background: token.colorFillAlter, border: `1px solid ${token.colorBorderSecondary}` }}>
          <Flex vertical gap={6}>
            <Text type="secondary" style={{ fontSize: 11 }}>{t('utilities.shortener.originalUrl')}</Text>
