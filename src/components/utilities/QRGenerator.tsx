...
import {
  Button, Flex, Image, Input, message,
  Select, Spin, Switch, Typography, theme,
} from 'antd'
...
export default function QRGenerator() {
  ...
  return (
    <Flex vertical gap={12}>
      {ctx}
      <Input
        size="large" placeholder={t('utilities.qr.placeholder')}
      ...
      <Flex align="center" gap={16} wrap="wrap">
        <Flex align="center" gap={6}>
          <Text type="secondary" style={{ fontSize: 13 }}>{t('utilities.qr.color')}:</Text>
        ...
        <Flex align="center" gap={6}>
          <Text type="secondary" style={{ fontSize: 13 }}>{t('utilities.qr.size')}:</Text>
          <Select value={size} onChange={setSize} size="small" style={{ width: 72 }}
            options={[5, 8, 10, 12, 15, 20].map(v => ({ label: `${v}×`, value: v }))}
          />
        </Flex>
        <Flex align="center" gap={6}>
          <Text type="secondary" style={{ fontSize: 13 }}>{t('utilities.qr.rounded')}:</Text>
      ...
      {mutation.isPending && (
        <Flex justify="center" align="center" vertical gap={8} style={{ padding: '24px 0' }}>
          <Spin />
          <Text type="secondary" style={{ fontSize: 13 }}>{t('utilities.qr.generating')}</Text>
        </Flex>
      )}
      ...
      {result && (
        <Flex vertical align="center" gap={12}>
          ...
          <Button icon={<DownloadOutlined />} onClick={download}>{t('utilities.qr.download')}</Button>
